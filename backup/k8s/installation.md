...
- 설치 직후 `kubectl get nodes -o wide`에서 외부망(`192.168.77.x`)이 잡히면 `node-ip` 강제 설정을 적용합니다.
...
- Synology NFS 권한에서 Kubernetes 노드 대역(`192.168.77.0/24`)이 허용되어 있어야 합니다.
...
- 예를 들어 `192.168.77.181`로 직접 접속하거나
  `192.168.77.180` VIP를 둘 계획이면 해당 주소를 모두
...
`INTERNAL-IP`가 외부망으로 잡힌 경우(예: `192.168.77.x`) 보정:
...
상세 절차는 별도 문서로 분리했습니다.

- 외부 API HA 문서:
  [Kubernetes External API HA](./external-api-ha.md)
- `vm-admin` 운영 노드 kubeconfig 전환:
  [VM-ADMIN Installation](../vm-admin/installation.md)
- 외부망 VIP를 사용하는 경우 SAN 반영:
  이 문서의 `apiserver SAN 변경 반영` 섹션 참고
...
외부망 `vm-admin`에서 `192.168.77.x` 대역으로 API에 직접 접근해야 한다면
...
- 이 절차의 직접 작업 대상은 `cp1`, `cp2`, `cp3`입니다.
- `w1`, `w2`는 일반적인 구성이라면 별도 수정이 필요하지 않습니다.
- `vm-admin`은 SAN 반영 이후 kubeconfig `server:` 값을 외부 운영 주소 또는 VIP 기준으로 확인해야 합니다.
...
- 최종적으로는 `192.168.77.180` 같은 외부망 VIP를 구성하고
...
- 노드 `INTERNAL-IP`가 외부망(`192.168.77.x`)으로 잡힌 상태를 방치하지 않습니다.
