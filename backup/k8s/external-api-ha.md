...
```text
VM-ADMIN (192.168.77.41)
        |
        v
K8s API VIP (192.168.77.180:6443)
        |
   +----+----+
   |         |
CT-LB1    CT-LB2
...
(HAProxy + Keepalived)
        |
        v
CP1 192.168.77.181:6443
CP2 192.168.77.182:6443
CP3 192.168.77.183:6443
```
...
- `vm-admin`의 kubeconfig는 최종적으로 `192.168.77.180:6443`를 사용해야 합니다.
- 외부망 VIP `192.168.77.180`은 apiserver `certSANs`에 포함되어 있어야 합니다.
...
- RAM: `512MB`
- Disk: `8GB`
- NIC: 외부망 `192.168.77.x`

예시:

- `ct-lb1`: `192.168.77.161`
- `ct-lb2`: `192.168.77.162`
- API VIP: `192.168.77.180`
...
- `ct-lb1`
  - CT ID: `121`
  - Hostname: `ct-lb1`
  - IP: `192.168.77.161/24`
  - Gateway: 외부망 기본 게이트웨이
- `ct-lb2`
  - CT ID: `122`
  - Hostname: `ct-lb2`
  - IP: `192.168.77.162/24`
  - Gateway: 외부망 기본 게이트웨이
...
ping -c 2 192.168.77.182
ping -c 2 192.168.77.183
```
...
```bash
ssh semtl@192.168.77.161
ssh semtl@192.168.77.162
sudo whoami
```
...
haproxy -v
keepalived --version
nc -vz 192.168.77.181 6443
nc -vz 192.168.77.182 6443
nc -vz 192.168.77.183 6443
```
...
backend k8s_api_back
  ...
  server cp1 192.168.77.181:6443 check
  server cp2 192.168.77.182:6443 check
  server cp3 192.168.77.183:6443 check
EOF
```
...
```bash
ping -c 2 192.168.77.180
nc -vz 192.168.77.180 6443
curl -k https://192.168.77.180:6443/version
```

정상 기준:

- `192.168.77.180` VIP가 `ct-lb1` 또는 `ct-lb2` 한 대에만 바인딩됨
- `nc -vz 192.168.77.180 6443` 연결 성공
- `curl -k https://192.168.77.180:6443/version` 응답 확인 가능
...
sed -i \
  's#server: https://.*:6443#server: https://192.168.77.180:6443#g' \
  ~/.kube/config
grep server ~/.kube/config
kubectl get nodes -o wide
```

`x509` 오류가 발생하면 apiserver 인증서 SAN에 `192.168.77.180`이 포함되어 있는지 확인해야 합니다.
상세 절차는
...
- `192.168.77.180` VIP가 한쪽 CT에 정상 바인딩됨
- `vm-admin`에서 `nc -vz 192.168.77.180 6443` 연결 성공
...
- `ct-lb1`:
  ...
  `- hostname : ct-lb1`
  `- vip : 192.168.77.180`
  `- backend : 192.168.77.181, 192.168.77.182, 192.168.77.183`
  `- haproxy : configured`
