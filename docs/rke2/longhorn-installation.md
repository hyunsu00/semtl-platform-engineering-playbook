...
이 문서는 이 저장소의 `RKE2` 표준 클러스터에 `Longhorn`을 설치하는 절차를 정리합니다.
...
- 클러스터 구성은 `control-plane 1대`, `worker 3대`입니다.
- 모든 노드에 `open-iscsi`가 설치되어 있고 `iscsid`가 활성화되어 있습니다.
- `nfs-common`은 Longhorn `RWX` 또는 백업 기능까지 사용할 계획이면 함께 준비합니다.
...
- `RWX` 볼륨 또는 NFS 기반 기능을 사용할 계획이면 각 노드에 `nfs-common`이 준비되어 있어야 합니다.
- `longhorn`이 지원하는 Kubernetes 버전 범위에 현재 RKE2 버전이 포함되어 있어야 합니다.
...
- `vm-rke2-cp1`, `vm-rke2-w1`, `vm-rke2-w2`, `vm-rke2-w3` 모두 Longhorn 전제 조건은 준비합니다.
...
- `vm-rke2-cp1`: VM 디스크 `100GB`
- `vm-rke2-w1`, `vm-rke2-w2`, `vm-rke2-w3`: VM 디스크 `300GB` 이상
- `worker` 노드 디스크는 운영 핵심 스택 규모를 감안하면 `400GB`부터 시작하는 편이 더 여유롭습니다.

판단 기준:

- `100GB`인 `control-plane` 노드는 OS, 컨테이너 런타임, 로그, Kubernetes 시스템
  리소스를 감당하는 용도로는 가능하지만, Longhorn 주 저장소 역할까지 맡기기에는
  여유가 작습니다.
- `300GB` 이상인 `worker` 노드는 Longhorn replica와 운영 핵심 스택을 올리기에
  더 현실적인 시작점입니다.
- `Prometheus`, `Grafana`, 이후 `Rancher`까지 Longhorn에 둘 계획이면
  `worker 400GB`로 시작하는 구성이 더 안정적입니다.

권장 방향:

- 현재 구성인 `cp 100GB`, `worker 300GB 이상`을 Longhorn 기본 기준으로 사용합니다.
...
`nfs-common`은 Longhorn `RWX` 또는 백업 기능을 사용할 때 함께 확인합니다.
이 문서에서는 설치 대신 상태만 점검합니다.
...
`RWX` 또는 NFS 기반 기능까지 사용할 계획이면 아래도 함께 확인합니다.
...
가장 단순한 성공 경로는 공식 릴리스 manifest를 적용하는 방식입니다.
`master` 브랜치를 직접 참조하지 말고, 검증된 버전을 고정합니다.
...
- `v1.10.1`은 현재 Longhorn 최신 안정 릴리스 기준입니다.
- 설치 전 현재 RKE2 Kubernetes 버전이 Longhorn 지원 범위에 포함되는지 함께 확인합니다.
...
필요하면 아래처럼 생성 과정을 잠시 watch 해도 됩니다.
...
- Longhorn 설치 직후 첫 PVC 생성은 수십 초 정도 `Pending`으로 보일 수 있습니다.
- `kubectl get pvc longhorn-test-pvc -w`로 `Bound` 전환까지 잠시 기다린 뒤 판단하는 편이 안전합니다.
...
- 현재 표준 디스크 크기는 `control-plane 100GB`, `worker 300GB 이상`입니다.
- 가능하면 Longhorn replica는 `worker`에 우선 배치하고, `control-plane`에는 최소화합니다.
...
- [Install Longhorn with kubectl](https://longhorn.io/docs/latest/deploy/install/install-with-kubectl/)
