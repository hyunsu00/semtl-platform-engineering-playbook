...
- 현재 문서 기준은 `proxy_download: true`이므로 GitLab VM이 MinIO 내부 API endpoint(`http://192.168.77.171:9000`)에 접근할 수 있으면 충분합니다.
- MinIO Console 도메인인 `https://minio.semtl.synology.me`는 GitLab object storage endpoint로 사용하지 않습니다.
...
gitlab_rails['object_store']['connection'] = {
  'provider' => 'AWS',
  'region' => 'us-east-1',
  'endpoint' => 'http://192.168.77.171:9000',
  'path_style' => true,
...
- 외부 브라우저 다운로드를 GitLab이 직접 처리하므로, 현재 문서 기준은 public S3 API endpoint보다
  내부 MinIO API endpoint를 사용하는 구성이 더 단순합니다.
