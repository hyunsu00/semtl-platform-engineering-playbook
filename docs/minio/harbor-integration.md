...
- Harbor는 내부/외부 Docker client가 blob push/pull에 관여하므로 `https://minio-api.semtl.synology.me` 같은 공개 S3 API endpoint를 권장합니다.
- MinIO Console 도메인인 `https://minio.semtl.synology.me`는 Harbor `regionendpoint`로 사용하지 않습니다.
...
storage_service:
  ...
  s3:
    ...
    regionendpoint: https://minio-api.semtl.synology.me
    # accesskey:
...
- `regionendpoint`는 `https://minio-api.semtl.synology.me`처럼 외부/내부에서 함께 접근 가능한 MinIO S3 API endpoint를 사용합니다.
...
- 내부/외부 Docker client가 함께 Harbor를 통해 이미지를 push/pull하는 환경이면 `redirect.disable: true`를 권장합니다.
- `redirect.disable: false`를 사용하려면 Docker client가 MinIO S3 endpoint에 직접 도달할 수 있어야 합니다.
- 내부 VM에서도 `minio-api.semtl.synology.me`가 정상 해석되고 TLS 인증서를 신뢰할 수 있어야 합니다.
- MinIO Console 도메인과 S3 API 도메인은 분리합니다. Harbor 연동에는 Console 도메인이 아니라 S3 API 도메인을 사용해야 합니다.
