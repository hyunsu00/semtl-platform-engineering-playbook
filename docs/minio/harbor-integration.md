...
storage_service:
  ...
  s3:
    ...
    # Console 포트(9001)가 아니라 API 포트(9000)를 사용해야 합니다.
  ...
  # redirect:
  # Registry redirect 동작을 비활성화하려면 true로 설정합니다.
  # 기본값 false는 redirect를 허용하는 의미입니다.
  redirect:
    disable: false
```
...
- `regionendpoint`는 MinIO S3 API(`9000`)를 사용합니다.
- `secure: false`는 내부 HTTP endpoint 기준입니다.
- Console 포트(`9001`)를 넣지 않습니다.
