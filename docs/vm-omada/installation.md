...
- hostname: `vm-omada`
- IP: `192.168.77.61`
- Omada URL: `https://192.168.77.61:8043`
...
- `http://192.168.77.61:8088`로 접속하면 보통 `https://192.168.77.61:8043/`로 이동합니다.
...
- 스냅샷 설명에는 `IP: 192.168.77.61`도 함께 남기면 추적이 편합니다.
- `Omada URL: https://192.168.77.61:8043`도 같이 적어두면 확인이 쉽습니다.
...
- 이 문서 예시는 로컬 LAN `192.168.77.0/24`, Alpine WireGuard VM `192.168.77.11`,
...
- `10.99.0.0/24 via 192.168.77.11 dev enp3s0` 같은 경로 확인
- `192.168.77.0/16 via 192.168.77.11 dev enp3s0` 같은 경로 확인
- `ip route get 192.168.77.61`는 로컬 NIC를 사용
- `ip route get 192.168.32.10`는 `via 192.168.77.11 dev enp3s0`로 표시
