...
- 연결 네트워크: `lan`
- 2.4GHz 변수: `wifi_2g_ssid`
- 5GHz 변수: `wifi_5g_ssid`
...
- `radio0.disabled`가 `0`
- `radio1.disabled`가 `0`
- `default_radio0.disabled`가 `1`
- `default_radio1.disabled`가 `1`
- `default_radio0.ssid`가 `wifi_2g_ssid` 값으로 렌더링됨
- `default_radio1.ssid`가 `wifi_5g_ssid` 값으로 렌더링됨
- `default_radio0.encryption`이 `none`으로 렌더링됨
- `default_radio1.encryption`이 `none`으로 렌더링됨

이 템플릿은 기본 OpenWrt Wi-Fi를 패스워드 없이 비활성화 상태로 관리합니다.
...
- 장비별 변수가 필요한 경우 2개 SSID 변수 모두 입력됨
...
- `wireless.default_radio0.ssid`가 2.4GHz 기본 SSID 값과 일치
- `wireless.default_radio1.ssid`가 5GHz 기본 SSID 값과 일치
...
성공 기준을 만족하지 않으면 `radio`, `band`, `disabled`, `ssid`, `encryption`,
`network` 값을 다시 확인합니다.
