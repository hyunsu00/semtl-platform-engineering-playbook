...
192.168.77.10 wg.internal.semtl.synology.me vm-wg
...
```bash
ip route add 192.168.32.0/24 via 192.168.77.10 dev ovs_eth0 2>/dev/null
```
...
- `192.168.32.0/24 via 192.168.77.10` 경로 확인
...
- `ip route`에 `192.168.32.0/24 via 192.168.77.10`가 보이는지 확인
