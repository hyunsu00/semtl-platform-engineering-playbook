...
- 기존 예시 `# AllowedIPs = 10.99.0.0/24, 192.168.200.0/24`는 IoT 대역만 터널로
  보내는 최소 구성 예시입니다.
- 이 예시에서는 WireGuard 터널 대역 `10.99.0.0/24`와 여러 원격 LAN 대역
  `192.168.1.0/24`, `192.168.10.0/24`, `192.168.20.0/24`, `192.168.100.0/24`,
  `192.168.110.0/24`, `192.168.200.0/24`, `192.168.210.0/24`를 터널로 보냅니다.
...
```bash
ip route add 192.168.200.0/24 via 192.168.77.10 dev ovs_eth0 2>/dev/null
```
...
- `192.168.200.0/24 via 192.168.77.10` 경로 확인
...
- `ip route`에 `192.168.200.0/24 via 192.168.77.10`가 보이는지 확인
