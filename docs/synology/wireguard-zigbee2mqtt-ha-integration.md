...
-> Zigbee Bridge (tcp://192.168.32.192:8888)
```

## 아키텍처와 예시 값

- Synology NAS IP: `192.168.77.2`
- Alpine WireGuard VM IP: `192.168.77.11`
- MQTT Broker IP: `192.168.77.21`
- Home Assistant IP: `192.168.77.21`
- Omada 공인 진입 주소 예시: `djcamp.ddnsfree.com:51820/udp`
- WireGuard 터널 대역 예시: `10.99.0.0/24`
- Alpine VM 터널 IP 예시: `10.99.0.4/24`
- 원격 Zigbee 대역: `192.168.32.0/24`
- 원격 Zigbee Bridge: `192.168.32.192:8888`
- Zigbee2MQTT UI 포트: `8099/tcp`

운영 원칙:

- `WireGuard`는 NAS 컨테이너가 아니라 별도 Alpine VM에서 종료
- NAS는 필요한 원격 사설 대역을 Alpine VM으로 보냄
...
- 원격 Zigbee Bridge가 `tcp://192.168.32.192:8888`로 접근 가능한 상태
- MQTT Broker 준비 완료
...
- 관리 IP가 `192.168.77.11/24`
- 기본 게이트웨이가 `192.168.77.1`
- DNS가 `192.168.77.2`
...
192.168.77.11 wg.internal.semtl.synology.me vm-wg
...
# AllowedIPs = 10.99.0.0/24, 192.168.32.0/24
AllowedIPs = 10.99.0.0/24, 192.168.32.0/24
...
- `AllowedIPs`는 Alpine VM이 WireGuard 피어 쪽으로 보낼 목적지 대역을 의미합니다.
- 기존 예시 `# AllowedIPs = 10.99.0.0/24, 192.168.32.0/24`는 IoT 대역만 터널로
  보내는 최소 구성 예시입니다.
- 여러 원격 LAN 대역을 함께 터널로 보내야 하면 `AllowedIPs`에 필요한 대역을
  쉼표로 추가합니다.
- 예시 대상은 `192.168.30.0/24`, `192.168.31.0/24`, `192.168.32.0/24`,
  `192.168.33.0/24`, `192.168.40.0/24`, `192.168.41.0/24`,
  `192.168.42.0/24`, `192.168.43.0/24`입니다.
- 즉, IoT 대역 이외의 다른 원격 대역도 WireGuard를 통해 통과시키기 위해
  `AllowedIPs`를 여러 대역으로 확장한 예시입니다.
- 이 문서에서는 NAS와 Alpine VM의 일반 인터넷 트래픽까지 터널로 보내지 않고,
  필요한 원격 사설 대역만 터널로 보내는 split tunnel 구성을 전제로 합니다.
- 따라서 `AllowedIPs = 0.0.0.0/0`는 이 문서 목적에 맞지 않습니다.
...
캡션: `SEMTL-NAS` 피어 예시, 인터페이스 `DJCAMP_WGVPN`, 주소 허용 `10.99.0.4/32`, `192.168.77.0/24`
...
- 기본값은 `주소 허용 1: 10.99.0.4/32`만으로도 됩니다.
- 이 문서처럼 Synology NAS가 Alpine VM을 거쳐 원격 Zigbee 대역으로만 나가고,
  원격 측에서 NAS 대역 `192.168.77.0/24`로 역방향 접근할 필요가 없다면
  `주소 허용 2: 192.168.77.0/24`는 없어도 됩니다.
- 반대로 원격 WireGuard 서버가 `192.168.77.0/24`를 Alpine VM 뒤 클라이언트
  대역으로 알아야 하거나, 원격 측에서 NAS LAN으로 되돌아오는 트래픽을 라우팅해야
  한다면 `주소 허용 2: 192.168.77.0/24`를 함께 넣습니다.
...
- NAS가 있는 로컬 LAN: `192.168.77.0/24` (필요한 경우만)

상황에 따라 서버 측에서 아래 둘 중 하나가 필요합니다.

- 라우팅 방식: `192.168.77.0/24` 대역을 Alpine VM 뒤 클라이언트로 인지
- NAT 방식: Alpine VM에서 MASQUERADE된 트래픽만 허용

판단 기준:

- 원격 Zigbee 대역으로 나가는 통신만 필요하면 `192.168.77.0/24` 광고는 보통 불필요
- 원격 측에서 NAS LAN으로 역방향 접근하거나 응답 경로를 명시적으로 보장해야 하면
  `192.168.77.0/24` 광고 필요
...
- Alpine VM에서 `ping 192.168.32.192` 또는 `nc -vz 192.168.32.192 8888` 성공
...
```bash
ip route add 192.168.32.0/24 via 192.168.77.11 dev ovs_eth0 2>/dev/null
```

운영 메모:

- 이 문서 기준 권장 방식은 `작업 스케줄러`입니다.
- 재부팅 후에도 `192.168.32.0/24` 경로를 일관되게 복구할 수 있습니다.
...
- `192.168.32.0/24 via 192.168.77.11` 경로 확인
...
- `ip route`에 `192.168.32.0/24 via 192.168.77.11`가 보이는지 확인
...
### 3. Alpine VM에서 `192.168.32.192:8888`가 안 열림

점검:

- Omada 측 ACL/방화벽
- 원격 Zigbee Bridge 서비스 상태
- `AllowedIPs`에 `192.168.32.0/24` 포함 여부

### 4. Zigbee2MQTT UI는 뜨는데 장치가 안 올라옴

점검:

- `serial.port: tcp://192.168.32.192:8888`
- `adapter` 타입
