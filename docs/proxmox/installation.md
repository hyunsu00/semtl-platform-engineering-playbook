...
- Proxmox 관리 IP 예시: `192.168.77.241`
- Gateway 예시: `192.168.77.1`
- 내부 DNS 예시: `192.168.77.2`
...
- Management IP: `192.168.77.241/24`
- Gateway: `192.168.77.1`
- DNS: `192.168.77.2`
- Bridge uplink NIC: `eth1` (USB LAN)

USB LAN 기준:

- USB NIC 기존 이름: `enx3c18a025c940`
- USB NIC MAC 주소: `3c:18:a0:25:c9:40`
- 고정할 이름: `eth1`

AMT 전용 내장 NIC 기준:

- Proxmox 표시 이름: `nic0`
- Linux 대체 이름: `enp0s31f6`, `enx002b675501fc`
- MAC 주소: `00:2b:67:55:01:fc`
- AMT 관리 IP: `192.168.77.254`
- 용도: Intel AMT 전용
- Proxmox 관리 브리지(`vmbr0`)에는 연결하지 않음
...
- DNS를 공유기가 아니라 내부 DNS 서버(`192.168.77.2`)로 직접 지정하는 구성을
  권장합니다.
...
- IP Address (CIDR): `192.168.77.241`
- CIDR: `24`
- Gateway: `192.168.77.1`
...
- 설치 화면상 NIC 이름은 `nic0`처럼 보일 수 있으며, 설치 후 Linux 내부
  인터페이스 이름은 `enp0s31f6`처럼 보일 수 있습니다.
- 관리 인터페이스는 이후 `vmbr0` 브리지의 uplink가 될 NIC를 선택합니다.
- USB LAN을 관리 uplink로 사용할 경우 MAC 주소 기반 `.link` 파일로
  `eth1` 이름을 고정한 뒤 `vmbr0`에 연결합니다.
- `nic0`(`enp0s31f6`)는 AMT 전용으로 유지하고 Proxmox 관리 IP를
  부여하지 않습니다.
...
- Web UI: `https://192.168.77.241:8006`
...
## 5. USB LAN 이름 고정과 vmbr0 연결

USB LAN을 Proxmox 관리망 uplink로 사용할 경우 장치 인식 순서에 의존하지
않도록 MAC 주소 기준으로 인터페이스 이름을 고정합니다.

현재 확인된 USB NIC:

```text
enx3c18a025c940
MACAddress=3c:18:a0:25:c9:40
```

먼저 USB NIC 링크를 올려 장치가 정상 동작하는지 확인합니다.

```bash
ip link set enx3c18a025c940 up
ip -br link show enx3c18a025c940
```

정상 기준:

- `enx3c18a025c940`가 `UP` 상태로 표시됨
- 케이블과 스위치 포트가 정상이라면 `LOWER_UP`도 함께 표시됨

`systemd-networkd`의 `.link` 파일을 생성합니다.

```bash
nano /etc/systemd/network/10-usb-net.link
```
...
[Link]
Name=eth1
AlternativeName=enx3c18a025c940
```
...
재부팅 후 USB NIC 이름이 `eth1`로 고정됐는지 확인합니다.
...
- `eth1`이 존재함
- `altname enx3c18a025c940`가 표시됨

그 다음 `/etc/network/interfaces`에서 `vmbr0`의 uplink를 `eth1`로
맞춥니다.
...
- `eth1`이 존재함
- `vmbr0`에 `192.168.77.241/24`가 설정됨
- `bridge link`에서 `eth1 master vmbr0`가 확인됨
- Web UI가 `https://192.168.77.241:8006`에서 열림

주의:

- `ip link set enx3c18a025c940 name eth1` 방식은 재부팅 후 원복될 수
  있으므로 운영 기준으로 사용하지 않습니다.
- 내장 NIC가 AMT와 공유되는 환경에서는 내장 NIC를 `vmbr0` uplink로
  사용하지 않고 USB LAN을 `eth1`로 분리합니다.
- `nic0`는 `192.168.77.254` AMT 전용 경로로만 사용하고, Proxmox Web UI와
  SSH 접속은 `192.168.77.241`의 `vmbr0`를 사용합니다.
...
설치 과정에서 입력한 DNS가 `/etc/resolv.conf`에 반영됐는지 확인합니다.
...
외부 보조 DNS가 필요하면 운영 기준에 따라 `1.1.1.1` 같은 보조
`nameserver`를 추가할 수 있습니다.
...
```bash
ssh semtl@192.168.77.241
ssh root@192.168.77.241
```

기대 결과:

- `ssh semtl@192.168.77.241` 접속 성공
- `ssh root@192.168.77.241` 접속 실패
...
### 11-4. Proxmox 관리자 계정 `admin@pve` 생성

Proxmox Web UI 전용 관리자 계정은 Linux 계정과 별도로 `pve` realm에
생성합니다.

```bash
pveum user add admin@pve --password '<change-required>'
