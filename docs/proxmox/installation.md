...
- Proxmox 관리 IP 예시: `192.168.0.241`
...
- Management IP: `192.168.0.241/24`
- Gateway: `192.168.0.1`
- DNS: `192.168.0.2`
...
- Web UI: `https://192.168.0.241:8006`
...
- `eth1`이 존재함
- `vmbr0`에 `192.168.0.241/24`가 설정됨
- `bridge link`에서 `eth1 master vmbr0`가 확인됨
- Web UI가 `https://192.168.0.241:8006`에서 열림
...
- `nic0`는 `192.168.0.254` AMT 전용 경로로만 사용하고, Proxmox Web UI와
  SSH 접속은 `192.168.0.241`의 `vmbr0`를 사용합니다.
...
```bash
ssh semtl@192.168.0.241
ssh root@192.168.0.241
```

기대 결과:

- `ssh semtl@192.168.0.241` 접속 성공
- `ssh root@192.168.0.241` 접속 실패
